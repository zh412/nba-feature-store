...
The pipeline generates **90 player-level features per NBA game**.
...
4. Feature Assembly  
   Endpoint data is merged into a unified player-level feature set containing **86 statistics and contextual metrics per game**.  
   During ingestion, **4 additional player metadata attributes** from the player dimension table are joined to the dataset, resulting in **90 total features per player game** in the final feature store.
...
The table contains **90 player-level features** generated from multiple NBA Stats API endpoints.
...
These datasets are merged into a unified player-level feature set containing 86 statistics and contextual metrics for every player game.
...
The final feature store contains 90 player-level features per game, composed of 86 game statistics collected from NBA Stats API endpoints and 4 stable player attributes joined from the player dimension table.
...
The pipeline generates **over 86 player-level features per game** including traditional statistics, advanced metrics, usage statistics, and team context features.
...
By combining data from these endpoints, the pipeline generates a unified dataset containing **90 player-level features per game**, which are validated and stored in the BigQuery feature store for downstream analytics and modeling workflows.
